...
// });
  ...
  console.log("holdfdfg");
  console.log(this.prime);

  // fetch('https://maps.googleapis.com/maps/api/place/findplacefromtext/json?key=AIzaSyB190V6L_hdgXp8PJtbbPr5Tb0eJ16GYeA&input=' + encodeURI(this.prime) + '&inputtype=textquery&fields=formatted_address, name', {
  //   method: 'Post',
  //   headers: {
  //       'Content-Type': 'application/json'
  //   }, })
  ...
  //   });
  this.secondo.forEach(element => {
    fetch('https://maps.googleapis.com/maps/api/place/findplacefromtext/json?key=AIzaSyB190V6L_hdgXp8PJtbbPr5Tb0eJ16GYeA&input=' + encodeURI(element.name) + '&inputtype=textquery&fields=formatted_address, name', {
      method: 'Post',
      headers: {
          'Content-Type': 'application/json'
      }, })
      .then(response => {this.pass = response;console.log(this.pass)})
      // .then(place => {this.recs.push(encodeURI(place.candidates[0].formatted_address));
      //                 this.recsname.push(place.candidates[0].name);
      //                 console.log(this.recs);
      // })
      ;
  });
